(() => {
  ...
  style.textContent = `
    .cpe-fab { position: fixed; right: 18px; bottom: 18px; width: 58px; height: 58px; border-radius: 50%;
      border: none; background: #0b5ea8; color: #fff; font-size: 22px; cursor: pointer; z-index: 2147483647;
      box-shadow: 0 8px 20px rgba(0,0,0,.25); }
    .cpe-panel { position: fixed; right: 18px; bottom: 86px; width: min(360px, calc(100vw - 28px)); max-height: 72vh;
      overflow: auto; background: #fff; border: 1px solid #d9e4f2; border-radius: 14px; padding: 12px; z-index: 2147483647;
      box-shadow: 0 16px 40px rgba(0,0,0,.22); font-family: Arial, sans-serif; color: #14304e; }
    ...
    .cpe-btn { border: 1px solid #c9d8eb; background: #f4f8ff; color: #14304e; border-radius: 10px; padding: 8px 10px; cursor: pointer; font-weight: 700; }
    .cpe-btn.primary { background: #0b5ea8; color: #fff; border-color: #0b5ea8; }
    .cpe-btn.small { font-size: 12px; padding: 6px 8px; }
    .cpe-input { width: 100%; box-sizing: border-box; border: 1px solid #c9d8eb; border-radius: 10px; padding: 9px 10px; margin-bottom: 8px; }
    .cpe-status { font-size: 12px; color: #395a79; margin-bottom: 8px; }
    .cpe-result { border: 1px solid #e1eaf6; border-radius: 10px; padding: 8px; }
  ...
  panel.innerHTML = `
    <div class="cpe-title">CPE Valencia</div>
    <div class="cpe-row">
      <button class="cpe-btn small" data-open="dondeVoy">📍 ¿Donde voy?</button>
      <button class="cpe-btn small" data-open="jornales">📋 Jornales</button>
      <button class="cpe-btn small" data-open="chapero">🚪 Chapero</button>
    </div>
    <div class="cpe-row">
      <button class="cpe-btn small" data-open="dobles">🔁 Dobles</button>
      <button class="cpe-btn small" data-open="pedirDescansos">🌴 Descansos</button>
    </div>
    <div class="cpe-row">
      <button class="cpe-btn small" data-external="miSueldo">💶 MiSueldoCPE</button>
      <button class="cpe-btn small" data-external="descansos">🗓️ DescansosCPE</button>
    </div>
    <input id="cpe-chapa" class="cpe-input" type="text" inputmode="numeric" placeholder="Chapa (5 digitos o 72999)" />
    <div class="cpe-row">
      <button id="cpe-calc" class="cpe-btn primary">Calcular puertas</button>
      <button id="cpe-close" class="cpe-btn">Cerrar</button>
    </div>
